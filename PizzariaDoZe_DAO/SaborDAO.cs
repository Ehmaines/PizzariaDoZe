    ...
    public class SaborDAO
    {
        ...
        public DataTable Buscar(Sabor sabor)
        {
            ...
            "SELECT s.id_sabor AS ID, s.descricao_sabor AS Nome, s.foto AS Foto, s.categoria AS Categoria, s.tipo AS Tipo, " +
                "(SELECT STRING_AGG(i.descricao_ingrediente, ', ') " +
                 "FROM itens_sabores AS iss " +
                 "INNER JOIN cad_ingredientes i ON i.id_ingrediente = iss.ingrediente_id " +
                 "WHERE iss.sabor_id = s.id_sabor) AS Ingredientes "+
